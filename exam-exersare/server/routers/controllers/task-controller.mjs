...
const createFeedback = async (req, res, next) =>{
  try{
    if(!req.params.uid){
      return res.status(400).json({message: 'User ID not found in path.'});
    }

    if(!req.params.tid){
      return res.status(400).json({message: 'Task ID not found in path.'});
    }
    const data = await models.Feedback.create({
      content: req.body.content,
      userId: parseInt(req.params.uid),
      taskId: parseInt(req.params.tid)
    })

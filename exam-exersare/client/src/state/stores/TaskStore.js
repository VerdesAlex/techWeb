...
class TaskStore {
  ...
  async postFeedback(state, projectId, taskId, content){
    try{
      const response = await fetch(`${SERVER}/api/users/${state.user.data.id}/projects/${projectId}/tasks/${taskId}/feedback`, {
      method: 'post',
      headers: {
        authorization: state.user.data.token,
        'Content-Type': 'application/json'
    ...
    // this.emitter.emit('POST_FEEDBACK_SUCCESS')
    this.getAllFeedback(state, projectId, taskId)


    }catch(err){
      console.warn(err)
      this.emitter.emit('POST_FEEDBACK_ERROR')
    }
  }

  async getAllFeedback(state, projectId, taskId, content){
    try{
      const response = await fetch(`${SERVER}/api/users/${state.user.data.id}/projects/${projectId}/tasks/${taskId}/feedback`, {
        headers: {
          authorization: state.user.data.token
